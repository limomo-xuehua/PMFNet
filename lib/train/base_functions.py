...
def get_optimizer_scheduler(net, cfg):
        ...
        param_dicts = [
            {
                ...
                "lr": cfg.TRAIN.LR  # prompt参数的学习率
            },
            #{
             #   "params": [p for n, p in net.named_parameters() if "cwa_fusion" in n and p.requires_grad],
              #  "lr": cfg.TRAIN.CWA_LR  # new_param参数的学习率
           # },
            {
                "params": [p for n, p in net.named_parameters() if
                           ("prompt" not in n and "cwa_fusion" not in n) and p.requires_grad],
    ...
    if cfg.TRAIN.SCHEDULER.TYPE == 'step':
        ...
        # ===================================== 新增：打印每个参数的学习率 =====================================
    if is_main_process():  # 仅主进程打印，避免多卡重复输出
        print("\n" + "=" * 80)
        print("All parameters and their learning rates (final effective values):")
        print("=" * 80)

        # 1. 建立「参数对象→参数名」的映射（关键：优化器中仅存参数对象，需反向匹配名称）
        param_name_map = {param: name for name, param in net.named_parameters()}

        # 2. 遍历优化器的所有参数分组，逐个打印参数名+学习率
        for group_idx, group in enumerate(optimizer.param_groups):
            group_lr = group["lr"]
            print(f"\n[Group {group_idx + 1}] Learning Rate: {group_lr:.6f}")
            print(f"Parameters in this group:")
            for param in group["params"]:
                param_name = param_name_map.get(param, "Unknown Parameter")
                print(f"  - {param_name}: lr={group_lr:.6f}")
        print("=" * 80 + "\n")
    # ===================================== 新增结束 =====================================
